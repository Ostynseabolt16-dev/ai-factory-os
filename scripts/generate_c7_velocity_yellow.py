#!/usr/bin/env python3
"""Generate Velocity Yellow C7 streetwear master — color variant, correct C7 body."""
...
STYLE: Smooth photoreal digital render. Clean glossy factory Velocity Yellow Tintcoat paint — vivid bright Corvette yellow with soft natural highlights and rich golden undertone (NOT lime green, NOT mustard, NOT orange, NOT pale cream). Sharp crisp details on the car. NO halftone dots, NO stipple grain, NO newspaper texture on the car body. Distressed weathered texture ONLY on the huge "C7" letters behind the car — NOT on the V8 letters.

LAYOUT (match proven C7 gothic car tee template):
...
- Quote is added in post-production via scripts/overlay_collection_quote.py
...
- Entire body painted glossy Velocity Yellow Tintcoat — hood, fenders, doors, roof all yellow
...
Centered composition, high contrast for small Etsy thumbnail — yellow car pops on white. No mockup, no t-shirt, no watermark.
""".strip()

DEFAULT_OUT = Path("designs/corvette/c7_velocity_yellow_collection.png")
...
def main() -> None:
    ...
    parser.add_argument("--draft-only", action="store_true", help="Save as _c7_velocity_yellow_attemptN.png only")
    args = parser.parse_args()

    if args.draft_only:
        out = Path(f"designs/_c7_velocity_yellow_attempt{args.attempt}.png")
    else:
        out = args.out
