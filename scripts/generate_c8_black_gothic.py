#!/usr/bin/env python3
"""Generate C8 black gothic streetwear master."""
...
STYLE: Smooth photoreal digital render. Clean glossy factory black paint — deep jet black with bright silver edge highlights and natural reflections so the body stays readable on white. Sharp crisp details on the car. NO halftone dots, NO stipple grain, NO newspaper texture on the car body. Distressed weathered texture ONLY on the huge "C8" letters behind the car — NOT on the V8 letters.
...
- Entire body painted glossy black with reflective highlights
- Bright machined silver/chrome wheels (NOT black wheels) for contrast on white shirts
...
DEFAULT_OUT = Path("designs/corvette/c8_black_gothic_collection.png")
...
def main() -> None:
    ...
    out = Path(f"designs/_c8_black_attempt{args.attempt}.png") if args.draft_only else args.out
